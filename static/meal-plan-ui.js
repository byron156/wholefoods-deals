(function () {
  ...
  form.elements.prime.checked = Boolean(saved.options?.prime);
  function options() {
  ...
  function persist() {
    try {localStorage.setItem(key,JSON.stringify({options:plan.options,signature,checked:[...checked],pantry:[...pantry]}));} catch (_) {}
  }
  function renderGroceries() {
    ...
    document.getElementById('grocery-list').innerHTML = plan.groceries.map(item => {
      ...
      const status = pantry.has(item.id) ? 'Already in your pantry' : deal ? `<span class="deal-label">${deal.discount}% off · ${escape(deal.priceText)}</span> · ${escape(deal.retailer)}${deal.store_id ? ' · selected location' : ''}<br>${product}` : 'No matching sale found · buy at regular price';
  ...
  document.getElementById('grocery-list').addEventListener('change', event => {
    ...
    event.target.closest('.grocery-row').classList.toggle('checked',event.target.checked);persist();
  });
  ...
  document.getElementById('download-plan').addEventListener('click', () => {
    ...
    for (const item of plan.groceries) lines.push(`${checked.has(item.id) ? '[x]' : '[ ]'} ${item.name}: ${amount(item)}${pantry.has(item.id) ? ' — ALREADY IN PANTRY' : item.deal ? ` — ${item.deal.retailer}: ${item.deal.name}, ${item.deal.priceText} per listed offer (${item.deal.discount}% off)` : ' — no matching sale; regular price'}`);
